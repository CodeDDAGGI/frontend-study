...
clockContainer.innerHTML =
    ...
    form.addEventListener('submit',(event)=>{
      ...
      // : 웹 페이지의 로컬 스토리지에 기존에 저장된 할 일 목록(배열)이 존재한다면 taskArr배열에 저장
      ...
      // includes 메서드: 매개변수값이 해당 배열에 존재하는지 여부를 boolean 값으로 반환
    ...
    toDoList.addEventListener('click', (e)=>{
      ...
      const lit = [...document.getElementsByClassName('task')];
    ...
    function updateProgress() {
      ...
      // 진행 상태 바의 너비 계산
      let newWidht = `${(count / list.lenght) * 100}%`;

      // 계산된 너비가 'NaN' 또는 '0'%인 경우 너비를 100%로 설정
      if(newWidht === 'NaN%' || newWidht === '0%') progressDone.style.widht = '100%';
      else progressDone.style.widht = newWidht;
