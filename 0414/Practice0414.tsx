...
function DiscountMessage({isDiscountActive}){
  ...
  return (
    {isDiscountActive ? <strong>할인 중</strong> : "현재 할인 되지 않음."}
...
const filteredAndSorted = employees.filter(employee => employee.role === 'developer' && employee.experience >= 3).sort((a,b) =>b.experience - a.experience).map(employee => (
