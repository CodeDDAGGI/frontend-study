...
const toggleOn = document.querySelector('.tog-on');
if(toggleOn !== null){
...
const toggleOff = document.querySelector('.tog-off');
if(toggleOff !== null){
...
function loadTodos(): void {
    ...
    savedTodos.forEach(todoText => {
        ...
        const deleteButton = document.createElement('button');
        const completedBtn = document.createElement('com-button')
        completedBtn.textContent = 'O';
        deleteButton.textContent = 'X';
        deleteButton.onclick = () => {
            todoList.removeChild(li);
            saveTodos();
...
function loadComplete():void {
  const completedTodos : string[] = JSON.parse(localStorage.getItem('todos') || '[]');
  const todoList = document.getElementById('todoList') as HTMLUListElement;
  completedTodos.forEach(todoText => {
    const comli = document.createElement('li2');
    comli.textContent = todoText;

    const comdeleteButton = document.createElement('combutton');
    comdeleteButton.textContent = "X";
    comdeleteButton.onclick = () => {
      todoList.removeChild(comli);
      saveTodos();
    }  ;

    comli.appendChild(comdeleteButton);
    todoList.appendChild(comli);
  });
}
